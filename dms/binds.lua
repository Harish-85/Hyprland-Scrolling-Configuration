...
-- FORCE QUIT
hl.bind("SUPER + SHIFT + CTRL + ESCAPE" ,hl.dsp.window.kill())
-- kill cursor
hl.bind("SUPER + SHIFT + ESCAPE",hl.dsp.exec_cmd("hyprctl kill"))


-- PASS THORUGHS for push to talk and stuff
-- Passes Left Alt directly down to Discord without blocking its desktop modifier behaviors
-- Corrected Lua syntax: Calling hl.dsp.pass() directly feeds the required table into the bind
-- hl.bind("V", hl.dsp.pass({ window = "class:discord" }))
-- hl.bind( "Alt_L", hl.dsp.pass({ window = "class:^(discord)$" }), { release = true })
